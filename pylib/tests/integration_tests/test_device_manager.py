...
def run_daemon(daemon_dir, driver_dir):
    ...
    daemon = openrazer_daemon.daemon.RazerDaemon(
        verbose=True, console_log=False, test_dir=driver_dir)
    daemon.run()
...
class DeviceManagerTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls._bw_chroma = fake_driver.FakeDevice(
            'razerblackwidowchroma', serial=cls._bw_serial, tmp_dir=cls._tmp_dir)
        print("Created BlackWidow Chroma endpoints")

        cls._daemon_proc = multiprocessing.Process(
            target=run_daemon, args=(cls._daemon_dir, cls._tmp_dir))
    ...
    def test_fw_version(self):
        ...
        self.assertEqual(device.firmware_version,
                         self._bw_chroma.get('firmware_version'))
    ...
    def test_device_keyboard_macro_mode(self):
        ...
        self.assertEqual(device.macro_mode_led_effect,
                         openrazer.client.constants.MACRO_LED_STATIC)
        device.macro_mode_led_effect = openrazer.client.constants.MACRO_LED_BLINK
        self.assertEqual(self._bw_chroma.get('mode_macro'), str(
            openrazer.client.constants.MACRO_LED_BLINK))
    ...
    def test_device_keyboard_effect_wave(self):
        ...
        self.assertEqual(self._bw_chroma.get('matrix_effect_wave'),
                         str(openrazer.client.constants.WAVE_LEFT))
        device.fx.wave(openrazer.client.constants.WAVE_RIGHT)
        self.assertEqual(self._bw_chroma.get('matrix_effect_wave'),
                         str(openrazer.client.constants.WAVE_RIGHT))
    ...
    def test_device_keyboard_effect_static(self):
        ...
        self.assertEqual(b'\xFF\x00\xFF', self._bw_chroma.get(
            'matrix_effect_static', binary=True))
    ...
    def test_device_keyboard_effect_reactive(self):
        ...
        self.assertEqual(b'\x01\xFF\x00\xFF', self._bw_chroma.get(
            'matrix_effect_reactive', binary=True))
    ...
    def test_device_keyboard_effect_breath_single(self):
        ...
        self.assertEqual(b'\xFF\x00\xFF', self._bw_chroma.get(
            'matrix_effect_breath', binary=True))
    ...
    def test_device_keyboard_effect_breath_dual(self):
        ...
        self.assertEqual(b'\xFF\x00\xFF\xFF\x00\x00', self._bw_chroma.get(
            'matrix_effect_breath', binary=True))
    ...
    def test_device_keyboard_effect_ripple(self):
        ...
        custom_effect_payload = self._bw_chroma.get(
            'matrix_custom_frame', binary=True)
