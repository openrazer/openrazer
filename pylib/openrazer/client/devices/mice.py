...
class RazerMouse(__RazerDevice):
    ...
    def __init__(self, serial, vid_pid=None, daemon_dbus=None):
        super(RazerMouse, self).__init__(
            serial, vid_pid=vid_pid, daemon_dbus=daemon_dbus)

        # Capabilities
        self._capabilities['poll_rate'] = self._has_feature(
            'razer.device.misc', ('getPollRate', 'setPollRate'))
        self._capabilities['dpi'] = self._has_feature(
            'razer.device.dpi', ('getDPI', 'setDPI'))

        if self.has('dpi'):
            self._dbus_interfaces['dpi'] = _dbus.Interface(
                self._dbus, "razer.device.dpi")
    ...
    def dpi(self, value: tuple):
        ...
        if self.has('dpi'):
            if len(value) != 2:
                raise ValueError(
                    "DPI tuple is not of length 2. Length: {0}".format(len(value)))
            dpi_x, dpi_y = value

            if not isinstance(dpi_x, int) or not isinstance(dpi_y, int):
                raise ValueError("DPI X or Y is not an integer, X:{0} Y:{1}".format(
                    type(dpi_x), type(dpi_y)))

            if dpi_x < 0 or dpi_x > 16000:  # TODO add in max dpi option
                raise ValueError(
                    "DPI X either too small or too large, X:{0}".format(dpi_x))
            if dpi_y < 0 or dpi_y > 16000:  # TODO add in max dpi option
                raise ValueError(
                    "DPI Y either too small or too large, Y:{0}".format(dpi_y))
    ...
    def poll_rate(self, poll_rate: int):
        ...
        if self.has('poll_rate'):
            if not isinstance(poll_rate, int):
                raise ValueError(
                    "Poll rate is not an integer: {0}".format(poll_rate))
            if poll_rate not in (_c.POLL_125HZ, _c.POLL_500HZ, _c.POLL_1000HZ):
                raise ValueError('Poll rate "{0}" is not one of {1}'.format(
                    poll_rate, (_c.POLL_125HZ, _c.POLL_500HZ, _c.POLL_1000HZ)))
