...
{
	return razer_get_usb_response(usb_dev, 0x00, request_report, 0x00,
				      response_report, RAZER_MOUSE_WAIT_MIN_US,
 ...
 */
struct razer_report razer_send_payload(struct usb_device *usb_dev,
				       struct razer_report *request_report)
{
	int retval = -1;
	struct razer_report response_report;
	...
	if (retval == 0) {
		...
		if (response_report.remaining_packets !=
			    request_report->remaining_packets ||
				     ...
				     const char *buf, size_t count)
{
	...
	struct razer_report report = razer_chroma_standard_set_led_state(
		VARSTORE, LOGO_LED, enabled);
					  ...
					  const char *buf, size_t count)
{
	...
	switch (usb_dev->descriptor.idProduct) {
	case USB_DEVICE_ID_RAZER_NAGA_HEX_V2:
		report = razer_chroma_mouse_extended_matrix_effect_none(
	...
	default:
		report = razer_chroma_standard_matrix_effect_none(
			VARSTORE, BACKLIGHT_LED);
		break;
	}
					    ...
					    const char *buf, size_t count)
{
	...
	struct usb_device *usb_dev = interface_to_usbdev(intf);
	struct razer_report report =
		razer_chroma_standard_matrix_effect_custom_frame(NOSTORE);

	switch (usb_dev->descriptor.idProduct) {
	case USB_DEVICE_ID_RAZER_NAGA_HEX_V2: // TODO look into this think its extended effects
		report = razer_chroma_standard_matrix_effect_custom_frame(
			NOSTORE);
		report.transaction_id.id = 0x3f;
		break;
					    ...
					    const char *buf, size_t count)
{
	...
	if (count == 3) {
		...
		default:
			report = razer_chroma_standard_matrix_effect_static(
				VARSTORE, BACKLIGHT_LED,
				(struct razer_rgb *)&buf[0]);
			break;
		}
		...
		printk(KERN_WARNING
		       "razermouse: Static mode only accepts RGB (3byte)");
	}
					  ...
					  const char *buf, size_t count)
{
	...
	struct razer_report report = razer_chroma_standard_matrix_effect_wave(
		VARSTORE, BACKLIGHT_LED, direction);
					      ...
					      const char *buf, size_t count)
{
	...
	switch (usb_dev->descriptor.idProduct) {
	case USB_DEVICE_ID_RAZER_NAGA_HEX_V2:
		report = razer_chroma_mouse_extended_matrix_effect_spectrum(
			VARSTORE, BACKLIGHT_LED);
		report.transaction_id.id = 0x3f;
		break;
	...
	default:
		report = razer_chroma_standard_matrix_effect_spectrum(
			VARSTORE, BACKLIGHT_LED);
		break;
	}
					      ...
					      const char *buf, size_t count)
{
	...
	if (count == 4) {
		...
		default:
			report = razer_chroma_standard_matrix_effect_reactive(
				VARSTORE, BACKLIGHT_LED, speed,
				(struct razer_rgb *)&buf[1]);
			break;
		}
		...
		printk(KERN_WARNING
		       "razermouse: Reactive only accepts Speed, RGB (4byte)");
	}
	return count;
}
					    ...
					    const char *buf, size_t count)
{
	...
	switch (usb_dev->descriptor
			.idProduct) { // TODO refactor to have 2 methods to split out the breathing crap
	case USB_DEVICE_ID_RAZER_NAGA_HEX_V2:
	case USB_DEVICE_ID_RAZER_NAGA_CHROMA:
		switch (count) {
		case 3: // Single colour mode
			report =
				razer_chroma_mouse_extended_matrix_effect_breathing_single(
					VARSTORE, BACKLIGHT_LED,
					(struct razer_rgb *)&buf[0]);
			break;
		...
		case 6: // Dual colour mode
			report =
				razer_chroma_mouse_extended_matrix_effect_breathing_dual(
					VARSTORE, BACKLIGHT_LED,
					(struct razer_rgb *)&buf[0],
		...
		default: // "Random" colour mode
			report =
				razer_chroma_mouse_extended_matrix_effect_breathing_random(
					VARSTORE, BACKLIGHT_LED);
			break;
		}

		if (usb_dev->descriptor.idProduct !=
		    USB_DEVICE_ID_RAZER_NAGA_CHROMA) { // Chroma uses ff
			report.transaction_id.id = 0x3f;
		}
		break;
	...
	default:
		...
		case 6: // Dual colour mode
			report =
				razer_chroma_standard_matrix_effect_breathing_dual(
					VARSTORE, BACKLIGHT_LED,
					(struct razer_rgb *)&buf[0],
					(struct razer_rgb *)&buf[3]);
			break;
 ...
 */
static ssize_t razer_attr_read_get_battery(struct device *dev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
 ...
 */
static ssize_t razer_attr_read_is_charging(struct device *dev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
 ...
 */
static ssize_t
razer_attr_write_set_charging_effect(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	...
	if (count == 1) {
		...
		printk(KERN_WARNING
		       "razermouse: Incorrect number of bytes for setting the charging effect\n");
	}
	return count;
}
 ...
 */
static ssize_t
razer_attr_write_set_charging_colour(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	...
	struct razer_report report =
		razer_chroma_misc_set_dock_charge_type(0x01);
	razer_send_payload(usb_dev, &report);

	if (count == 3) {
		report = razer_chroma_standard_set_led_rgb(
			NOSTORE, BATTERY_LED, (struct razer_rgb *)&buf[0]);
		razer_send_payload(usb_dev, &report);
	} else {
		printk(KERN_WARNING
		       "razermouse: Charging colour mode only accepts RGB (3byte)");
	}
 ...
 * Returns a string
 */
static ssize_t razer_attr_read_poll_rate(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	struct razer_mouse_device *device = dev_get_drvdata(dev);
 ...
 * Sets the poll rate
 */
static ssize_t razer_attr_write_poll_rate(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
	struct razer_mouse_device *device = dev_get_drvdata(dev);
	unsigned short polling_rate =
		(unsigned short)simple_strtoul(buf, NULL, 10);
	struct razer_report report =
	...
	case USB_DEVICE_ID_RAZER_OROCHI_2011:
		...
		report = razer_chroma_misc_set_orochi2011_poll_dpi(
			device->orochi2011_poll, device->orochi2011_dpi,
...
static ssize_t razer_attr_write_set_brightness(struct device *dev,
					       struct device_attribute *attr,
					       const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	...
	case USB_DEVICE_ID_RAZER_OROCHI_CHROMA:
		...
		report = razer_chroma_standard_set_led_brightness(
			VARSTORE, SCROLL_WHEEL_LED, brightness);
		break;

	case USB_DEVICE_ID_RAZER_NAGA_HEX_V2:
		report = razer_chroma_standard_set_led_brightness(
			VARSTORE, BACKLIGHT_LED, brightness);
		report.transaction_id.id = 0x3f;
		break;
 ...
 * Returns a string
 */
static ssize_t razer_attr_read_set_brightness(struct device *dev,
					      struct device_attribute *attr,
					      char *buf)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	...
	case USB_DEVICE_ID_RAZER_OROCHI_CHROMA:
		// Orochi sets brightness of scroll wheel apparently
		report = razer_chroma_standard_get_led_brightness(
			VARSTORE, SCROLL_WHEEL_LED);
		break;
 ...
 */
static ssize_t razer_attr_write_mouse_dpi(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
	struct razer_mouse_device *device = dev_get_drvdata(dev);
	...
	case USB_DEVICE_ID_RAZER_DEATHADDER_2013:
		if (count == 1) {
			...
			printk(KERN_WARNING
			       "razermouse: DPI requires 1 byte or 2 bytes\n");
			return count;
		}

		report = razer_chroma_misc_set_dpi_xy_byte(dpi_x_byte,
							   dpi_y_byte);
		razer_send_payload(device->usb_dev, &report);
		return count;
		break;
	...
	case USB_DEVICE_ID_RAZER_OROCHI_2011:
		...
		report = razer_chroma_misc_set_orochi2011_poll_dpi(
			device->orochi2011_poll, dpi_x_byte, dpi_y_byte);
		razer_send_payload(device->usb_dev, &report);
		return count;
		break;
	...
	if (count != 2 && count != 4) {
		printk(KERN_WARNING
		       "razermouse: DPI requires 2 bytes or 4 bytes\n");
	} else {
		if (count == 2) {
			dpi_x = (buf[0] << 8) |
				(buf[1] &
				 0xFF); // TODO make convenience function
			report = razer_chroma_misc_set_dpi_xy(varstore, dpi_x,
							      dpi_x);
 ...
 */
static ssize_t razer_attr_read_mouse_dpi(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	struct razer_mouse_device *device = dev_get_drvdata(dev);
	...
	if (device->usb_pid == USB_DEVICE_ID_RAZER_NAGA_HEX ||
	    ...
	    device->usb_pid ==
		    USB_DEVICE_ID_RAZER_ABYSSUS_1800) { // NagaHex is crap uses only byte for dpi
		dpi_x = response.arguments[0];
		dpi_y = response.arguments[1];
	} else {
		dpi_x = (response.arguments[1] << 8) |
			(response.arguments[2] &
			 0xFF); // Apparently the char buffer is rubbish, as buf[1] somehow can equal FFFFFF80????
		dpi_y = (response.arguments[3] << 8) |
			(response.arguments[4] & 0xFF);
	}
 ...
 */
static ssize_t razer_attr_write_set_idle_time(struct device *dev,
					      struct device_attribute *attr,
					      const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
 ...
 */
static ssize_t
razer_attr_write_set_low_battery_threshold(struct device *dev,
					   struct device_attribute *attr,
					   const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	...
	struct razer_report report =
		razer_chroma_misc_set_low_battery_threshold(threshold);
 ...
 * Writes the colour segments on the mouse.
 */
static ssize_t razer_attr_write_set_key_row(struct device *dev,
					    struct device_attribute *attr,
					    const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	...
	while (offset < count) {
		if (offset + 3 > count) {
			printk(KERN_ALERT
			       "razermouse: Wrong Amount of data provided: Should be ROW_ID, START_COL, STOP_COL, N_RGB\n");
			break;
		}
		...
		if (start_col > stop_col) {
			printk(KERN_ALERT
			       "razermouse: Start column is greater than end column\n");
			break;
		}

		if (offset + row_length > count) {
			printk(KERN_ALERT
			       "razermouse: Not enough RGB to fill row\n");
			break;
		}
		...
		case USB_DEVICE_ID_RAZER_DEATHADDER_ELITE:
			report = razer_chroma_extended_matrix_set_custom_frame(
				row_id, start_col, stop_col,
				(unsigned char *)&buf[offset]);
			report.transaction_id.id = 0x3f;
			break;
		...
		case USB_DEVICE_ID_RAZER_MAMBA_WIRED:
		case USB_DEVICE_ID_RAZER_MAMBA_WIRELESS:
			report = razer_chroma_misc_one_row_set_custom_frame(
				start_col, stop_col,
				(unsigned char *)&buf[offset]);
			report.transaction_id.id = 0x80;
			break;
 ...
 */
static ssize_t razer_attr_write_device_mode(struct device *dev,
					    struct device_attribute *attr,
					    const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	...
	if (count == 2) {
		...
		printk(KERN_WARNING
		       "razerkbd: Device mode only takes 2 bytes.");
	}
 ...
 * Returns a string
 */
static ssize_t razer_attr_read_device_mode(struct device *dev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
 ...
 */
static ssize_t
razer_attr_read_scroll_led_brightness(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	struct usb_device *usb_dev = interface_to_usbdev(intf);
	struct razer_report report = razer_chroma_standard_get_led_brightness(
		VARSTORE, SCROLL_WHEEL_LED);
	struct razer_report response;
 ...
 */
static ssize_t
razer_attr_write_scroll_led_brightness(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	...
	case USB_DEVICE_ID_RAZER_DEATHADDER_ELITE:
		report = razer_chroma_extended_matrix_brightness(
			VARSTORE, SCROLL_WHEEL_LED, brightness);
		report.transaction_id.id = 0x3F;
		break;
 ...
 */
static ssize_t
razer_attr_read_logo_led_brightness(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	struct usb_device *usb_dev = interface_to_usbdev(intf);
	...
	switch (usb_dev->descriptor.idProduct) {
	case USB_DEVICE_ID_RAZER_NAGA_HEX_V2:
		report = razer_chroma_standard_get_led_brightness(VARSTORE,
								  LOGO_LED);
		report.transaction_id.id = 0x3F;
		break;

	case USB_DEVICE_ID_RAZER_DEATHADDER_ELITE:
		report = razer_chroma_extended_matrix_get_brightness(VARSTORE,
								     LOGO_LED);
		report.transaction_id.id = 0x3F;
		break;
 ...
 */
static ssize_t
razer_attr_write_logo_led_brightness(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
 ...
 */
static ssize_t razer_attr_write_scroll_led_state(struct device *dev,
						 struct device_attribute *attr,
						 const char *buf, size_t count)
{
	struct razer_mouse_device *device = dev_get_drvdata(dev);
	unsigned char enabled = (unsigned char)simple_strtoul(buf, NULL, 10);
	struct razer_report report = razer_chroma_standard_set_led_state(
		VARSTORE, SCROLL_WHEEL_LED, enabled);
	report.transaction_id.id = 0x3F;
	...
	case USB_DEVICE_ID_RAZER_OROCHI_2011:
		...
		report = razer_chroma_misc_set_orochi2011_led(
			device->orochi2011_led);
		break;
	}
 ...
 */
static ssize_t razer_attr_read_scroll_led_state(struct device *dev,
						struct device_attribute *attr,
						char *buf)
{
	struct razer_mouse_device *device = dev_get_drvdata(dev);
	struct razer_report report =
		razer_chroma_standard_get_led_state(VARSTORE, SCROLL_WHEEL_LED);
	struct razer_report response;
	report.transaction_id.id = 0x3F;
					      ...
					      char *buf)
{
	struct razer_mouse_device *device = dev_get_drvdata(dev);
	struct razer_report report =
		razer_chroma_standard_get_led_state(VARSTORE, LOGO_LED);
	struct razer_report response;
	report.transaction_id.id = 0x3F;
 ...
 */
static ssize_t razer_attr_write_scroll_led_rgb(struct device *dev,
					       struct device_attribute *attr,
					       const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	...
	if (count == 3) {
		...
		printk(KERN_WARNING
		       "razermouse: Scroll wheel LED mode only accepts RGB (3byte)");
	}
 ...
 */
static ssize_t razer_attr_read_scroll_led_rgb(struct device *dev,
					      struct device_attribute *attr,
					      char *buf)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	struct usb_device *usb_dev = interface_to_usbdev(intf);
	struct razer_report report =
		razer_chroma_standard_get_led_rgb(VARSTORE, SCROLL_WHEEL_LED);
	struct razer_report response;
	report.transaction_id.id = 0x3F;
	response = razer_send_payload(usb_dev, &report);

	return sprintf(buf, "%u%u%u\n", response.arguments[2],
		       response.arguments[3], response.arguments[4]);
}

/**
 * Write device file "logo_led_rgb"
 */
static ssize_t razer_attr_write_logo_led_rgb(struct device *dev,
					     struct device_attribute *attr,
					     const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	...
	if (count == 3) {
		report = razer_chroma_standard_set_led_rgb(
			VARSTORE, LOGO_LED, (struct razer_rgb *)&buf[0]);
		report.transaction_id.id = 0x3F;
		razer_send_payload(usb_dev, &report);
	} else {
		printk(KERN_WARNING
 ...
 */
static ssize_t razer_attr_read_logo_led_rgb(struct device *dev,
					    struct device_attribute *attr,
					    char *buf)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	struct usb_device *usb_dev = interface_to_usbdev(intf);
	struct razer_report report =
		razer_chroma_standard_get_led_rgb(VARSTORE, LOGO_LED);
	struct razer_report response;
 ...
 */
static ssize_t razer_attr_write_scroll_led_effect(struct device *dev,
						  struct device_attribute *attr,
						  const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	...
	struct razer_report report = razer_chroma_standard_set_led_effect(
		VARSTORE, SCROLL_WHEEL_LED, effect);
	report.transaction_id.id = 0x3F;
 ...
 */
static ssize_t razer_attr_read_scroll_led_effect(struct device *dev,
						 struct device_attribute *attr,
						 char *buf)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	struct usb_device *usb_dev = interface_to_usbdev(intf);
	struct razer_report report = razer_chroma_standard_get_led_effect(
		VARSTORE, SCROLL_WHEEL_LED);
	struct razer_report response;
	report.transaction_id.id = 0x3F;
	response = razer_send_payload(usb_dev, &report);
 ...
 */
static ssize_t razer_attr_write_logo_led_effect(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
 ...
 */
static ssize_t razer_attr_read_logo_led_effect(struct device *dev,
					       struct device_attribute *attr,
					       char *buf)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	struct usb_device *usb_dev = interface_to_usbdev(intf);
	struct razer_report report =
		razer_chroma_standard_get_led_effect(VARSTORE, LOGO_LED);
	struct razer_report response;
 ...
 */
static ssize_t
razer_attr_write_scroll_mode_spectrum(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	...
	case USB_DEVICE_ID_RAZER_DEATHADDER_ELITE:
		report = razer_chroma_extended_matrix_effect_spectrum(
			VARSTORE, SCROLL_WHEEL_LED);
		report.transaction_id.id = 0x3f;
		break;
	}
 ...
 */
static ssize_t
razer_attr_write_scroll_mode_reactive(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	...
	if (count == 4) {
		...
		case USB_DEVICE_ID_RAZER_NAGA_HEX_V2:
			report =
				razer_chroma_mouse_extended_matrix_effect_reactive(
					VARSTORE, SCROLL_WHEEL_LED, speed,
					(struct razer_rgb *)&buf[1]);
			report.transaction_id.id = 0x3f;
			break;
 ...
 */
static ssize_t
razer_attr_write_scroll_mode_breath(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
 ...
 */
static ssize_t
razer_attr_write_scroll_mode_static(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
 ...
 * No effect is activated whenever this file is written to
 */
static ssize_t razer_attr_write_scroll_mode_none(struct device *dev,
						 struct device_attribute *attr,
						 const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
 ...
 */
static ssize_t
razer_attr_write_logo_mode_spectrum(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	...
	case USB_DEVICE_ID_RAZER_DEATHADDER_ELITE:
		report = razer_chroma_extended_matrix_effect_spectrum(VARSTORE,
								      LOGO_LED);
		report.transaction_id.id = 0x3f;
		break;
	}
 ...
 */
static ssize_t
razer_attr_write_logo_mode_reactive(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
 ...
 * Sets breathing mode by writing 1, 3 or 6 bytes
 */
static ssize_t razer_attr_write_logo_mode_breath(struct device *dev,
						 struct device_attribute *attr,
						 const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
 ...
 * Set the mouse to static mode when 3 RGB bytes are written
 */
static ssize_t razer_attr_write_logo_mode_static(struct device *dev,
						 struct device_attribute *attr,
						 const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
 ...
 * No effect is activated whenever this file is written to
 */
static ssize_t razer_attr_write_logo_mode_none(struct device *dev,
					       struct device_attribute *attr,
					       const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
	...
	case USB_DEVICE_ID_RAZER_DEATHADDER_ELITE:
		report = razer_chroma_extended_matrix_effect_none(VARSTORE,
								  LOGO_LED);
		report.transaction_id.id = 0x3f;
		break;
	}
 ...
 */
static ssize_t
razer_attr_write_backlight_led_state(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct razer_mouse_device *device = dev_get_drvdata(dev);
	unsigned char enabled = (unsigned char)simple_strtoul(buf, NULL, 10);
	struct razer_report report = razer_chroma_standard_set_led_state(
		VARSTORE, BACKLIGHT_LED, enabled);
	report.transaction_id.id = 0x3F;
 ...
 */
static ssize_t
razer_attr_read_backlight_led_state(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct razer_mouse_device *device = dev_get_drvdata(dev);
	struct razer_report report =
		razer_chroma_standard_get_led_state(VARSTORE, BACKLIGHT_LED);
	struct razer_report response;
	report.transaction_id.id = 0x3F;
...
static DEVICE_ATTR(firmware_version, 0440, razer_attr_read_get_firmware_version,
		   NULL);
static DEVICE_ATTR(test, 0220, NULL, razer_attr_write_test);
static DEVICE_ATTR(poll_rate, 0660, razer_attr_read_poll_rate,
		   razer_attr_write_poll_rate);
static DEVICE_ATTR(dpi, 0660, razer_attr_read_mouse_dpi,
		   razer_attr_write_mouse_dpi);
...
static DEVICE_ATTR(matrix_brightness, 0660, razer_attr_read_set_brightness,
...
static DEVICE_ATTR(scroll_led_brightness, 0660,
...
static DEVICE_ATTR(logo_led_brightness, 0660,
...
// For old-school led commands
static DEVICE_ATTR(backlight_led_state, 0660,
		   razer_attr_read_backlight_led_state,
		   razer_attr_write_backlight_led_state);

/**
 * Raw event function
 */
static int razer_raw_event(struct hid_device *hdev, struct hid_report *report,
			   u8 *data, int size)
{
 ...
 * Mouse init function
 */
void razer_mouse_init(struct razer_mouse_device *dev,
		      struct usb_interface *intf, struct hid_device *hdev)
{
	struct usb_device *usb_dev = interface_to_usbdev(intf);
	...
	if (intf->cur_altsetting->desc.bInterfaceProtocol ==
		...
		case USB_DEVICE_ID_RAZER_DEATHADDER_ELITE:
			...
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_logo_led_brightness);
			CREATE_DEVICE_FILE(
				&hdev->dev,
				&dev_attr_logo_matrix_effect_spectrum);
			CREATE_DEVICE_FILE(
				&hdev->dev,
				&dev_attr_logo_matrix_effect_reactive);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_logo_matrix_effect_breath);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_logo_matrix_effect_static);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_logo_matrix_effect_none);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_scroll_led_brightness);
			CREATE_DEVICE_FILE(
				&hdev->dev,
				&dev_attr_scroll_matrix_effect_spectrum);
			CREATE_DEVICE_FILE(
				&hdev->dev,
				&dev_attr_scroll_matrix_effect_reactive);
			CREATE_DEVICE_FILE(
				&hdev->dev,
				&dev_attr_scroll_matrix_effect_breath);
			CREATE_DEVICE_FILE(
				&hdev->dev,
				&dev_attr_scroll_matrix_effect_static);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_scroll_matrix_effect_none);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_matrix_effect_custom);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_matrix_custom_frame);
			break;
		...
		case USB_DEVICE_ID_RAZER_NAGA_CHROMA:
			...
			CREATE_DEVICE_FILE(&hdev->dev, &dev_attr_dpi);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_matrix_brightness);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_matrix_effect_spectrum);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_matrix_effect_reactive);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_matrix_effect_breath);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_matrix_effect_static);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_matrix_effect_none);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_logo_led_brightness);
			CREATE_DEVICE_FILE(
				&hdev->dev,
		...
		case USB_DEVICE_ID_RAZER_MAMBA_2012_WIRED:
			...
			CREATE_DEVICE_FILE(&hdev->dev, &dev_attr_poll_rate);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_charge_low_threshold);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_device_idle_time);
			CREATE_DEVICE_FILE(&hdev->dev, &dev_attr_dpi);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_scroll_led_brightness);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_scroll_led_state);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_scroll_led_rgb);
			CREATE_DEVICE_FILE(&hdev->dev,
					   &dev_attr_scroll_led_effect);
			break;
...
static const struct hid_device_id razer_devices[] = {
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_OROCHI_2011) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_ABYSSUS_1800) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_ABYSSUS_2000) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_NAGA_HEX_RED) },
	...
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_MAMBA_2012_WIRED) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_MAMBA_2012_WIRELESS) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_MAMBA_WIRED) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_MAMBA_WIRELESS) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_MAMBA_TE_WIRED) },
	...
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_DEATHADDER_2013) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_OROCHI_2013) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_OROCHI_CHROMA) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_DEATHADDER_CHROMA) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_NAGA_HEX_V2) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_NAGA_CHROMA) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_DEATHADDER_ELITE) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_DIAMONDBACK_CHROMA) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER, USB_DEVICE_ID_RAZER_ABYSSUS_V2) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_DEATHADDER_3500) },
	{}
};
