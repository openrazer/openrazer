		     ...
		     struct razer_report *response_report)
{
	return razer_get_usb_response(usb_dev, 0x00, request_report, 0x00,
				      response_report,
				      RAZER_FIREFLY_WAIT_MIN_US,
				      RAZER_FIREFLY_WAIT_MAX_US);
}
					      ...
					      const char *buf, size_t count)
{
	...
	if (count == 4) {
		...
		printk(KERN_WARNING
		       "razerfirefly: Reactive only accepts Speed, RGB (4byte)");
	}
	return count;
}
 ...
 */
static ssize_t
razer_attr_write_mode_reactive_trigger(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t count)
{
	struct usb_interface *intf = to_usb_interface(dev->parent);
					    ...
					    const char *buf, size_t count)
{
	...
	if (count == 3) {
		...
		printk(KERN_WARNING
		       "razerfirefly: Static mode only accepts RGB (3byte)");
	}
					    ...
					    const char *buf, size_t count)
{
	...
	while (offset < count) {
		if (offset + 3 > count) {
			printk(KERN_ALERT
			       "razerfirefly: Wrong Amount of data provided: Should be ROW_ID, START_COL, STOP_COL, N_RGB\n");
			break;
		}
...
static DEVICE_ATTR(matrix_reactive_trigger, 0220, NULL,
		   razer_attr_write_mode_reactive_trigger);
 ...
 * TODO remove goto's
 */
static int razer_firefly_probe(struct hid_device *hdev,
