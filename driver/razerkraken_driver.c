 ...
 */
void print_erroneous_kraken_request_report(
	struct razer_kraken_request_report *report, char *driver_name,
	char *message)
{
	printk(KERN_WARNING
	       "%s: %s. Report ID: %02x dest: %02x length: %02x ADDR: %02x%02x Args: %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x .\n",
	       driver_name, message, report->report_id, report->destination,
	       report->length, report->addr_h, report->addr_l,
	       ...
	       report->arguments[9], report->arguments[10],
	       report->arguments[11], report->arguments[12],
	       report->arguments[13], report->arguments[14],
	       report->arguments[15]);
}

int razer_kraken_send_control_msg(struct usb_device *usb_dev,
...
get_kraken_request_report(unsigned char report_id, unsigned char destination,
			  unsigned char length, unsigned short address)
{
	struct razer_kraken_request_report report;
...
unsigned char get_current_effect(struct device *dev)
{
	...
	struct razer_kraken_request_report report = get_kraken_request_report(
		0x04, 0x00, 0x01, device->led_mode_address);
...
unsigned int get_rgb_from_addr(struct device *dev, unsigned short address,
			       unsigned char len, char *buf)
{
	struct razer_kraken_device *device = dev_get_drvdata(dev);
	struct razer_kraken_request_report report =
		get_kraken_request_report(0x04, 0x00, len, address);
	int is_mutex_locked = mutex_is_locked(&device->lock);
					      ...
					      const char *buf, size_t count)
{
	struct razer_kraken_device *device = dev_get_drvdata(dev);
	struct razer_kraken_request_report report = get_kraken_request_report(
		0x04, 0x40, 0x01, device->led_mode_address);
					    ...
					    const char *buf, size_t count)
{
	...
	struct razer_kraken_request_report rgb_report =
		get_kraken_request_report(0x04, 0x40, count,
					  device->breathing_address[0]);
	struct razer_kraken_request_report effect_report =
		get_kraken_request_report(0x04, 0x40, 0x01,
					  device->led_mode_address);
	union razer_kraken_effect_byte effect_byte = get_kraken_effect_byte();
	...
	if (count == 3 || count == 4) {
		...
		case USB_DEVICE_ID_RAZER_KRAKEN:
		case USB_DEVICE_ID_RAZER_KRAKEN_V2:
			razer_kraken_send_control_msg(device->usb_dev,
						      &rgb_report, 0);
			break;
		}

		// Send Set static command
		razer_kraken_send_control_msg(device->usb_dev, &effect_report,
					      0);
		mutex_unlock(&device->lock);

	} else {
		printk(KERN_WARNING
		       "razerkraken: Static mode only accepts RGB (3byte) or RGB with intensity (4byte)\n");
	}
					    ...
					    const char *buf, size_t count)
{
	...
	if (count == 3 || count == 4) {
		...
		printk(KERN_WARNING
		       "razerkraken: Custom mode only accepts RGB (3byte) or RGB with intensity (4byte)\n");
	}
 ...
 */
static ssize_t razer_attr_read_mode_static(struct device *dev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct razer_kraken_device *device = dev_get_drvdata(dev);
 ...
 * Returns 4 bytes for config
 */
static ssize_t razer_attr_read_mode_custom(struct device *dev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct razer_kraken_device *device = dev_get_drvdata(dev);
					    ...
					    const char *buf, size_t count)
{
	...
	if (device->usb_pid == USB_DEVICE_ID_RAZER_KRAKEN && count != 3) {
		printk(KERN_WARNING
		       "razerkraken: Breathing mode only accepts RGB (3byte)\n");
		return count;
	}

	if (count == 3) {
		struct razer_kraken_request_report rgb_report =
			get_kraken_request_report(0x04, 0x40, 0x03,
						  device->breathing_address[0]);
		...
		struct razer_kraken_request_report rgb_report =
			get_kraken_request_report(0x04, 0x40, 0x03,
						  device->breathing_address[1]);
		struct razer_kraken_request_report rgb_report2 =
			get_kraken_request_report(
				0x04, 0x40, 0x03,
				device->breathing_address[1] +
					4); // Address the 2nd set of colours
		...
		struct razer_kraken_request_report rgb_report =
			get_kraken_request_report(0x04, 0x40, 0x03,
						  device->breathing_address[2]);
		struct razer_kraken_request_report rgb_report2 =
			get_kraken_request_report(
				0x04, 0x40, 0x03,
				device->breathing_address[2] +
					4); // Address the 2nd set of colours
		struct razer_kraken_request_report rgb_report3 =
			get_kraken_request_report(
				0x04, 0x40, 0x03,
				device->breathing_address[2] +
					8); // Address the 3rd set of colours
		...
		printk(KERN_WARNING
		       "razerkraken: Breathing mode only accepts RGB (3byte), RGB RGB (6byte) or RGB RGB RGB (9byte)\n");
	}
 ...
 */
static ssize_t razer_attr_read_mode_breath(struct device *dev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct razer_kraken_device *device = dev_get_drvdata(dev);
	union razer_kraken_effect_byte effect_byte =
		(union razer_kraken_effect_byte)get_current_effect(dev);
	...
	case USB_DEVICE_ID_RAZER_KRAKEN_V2:
		switch (num_colours) {
		case 3:
			return get_rgb_from_addr(
				dev, device->breathing_address[2], 0x0C, buf);
			break;
		case 2:
			return get_rgb_from_addr(
				dev, device->breathing_address[1], 0x08, buf);
			break;
		default:
			return get_rgb_from_addr(
				dev, device->breathing_address[0], 0x04, buf);
			break;
		}
		break;
	...
	case USB_DEVICE_ID_RAZER_KRAKEN:
	default:
		return get_rgb_from_addr(dev, device->breathing_address[0],
					 0x04, buf);
		break;
	}
}
					  ...
					  char *buf)
{
	struct razer_kraken_device *device = dev_get_drvdata(dev);
	struct razer_kraken_request_report report =
		get_kraken_request_report(0x04, 0x20, 0x16, 0x7f00);
	...
	if (device->serial[0] == '\0') {
		...
		if (device->data[0] == 0x05) {
			...
			printk(KERN_CRIT
			       "razerkraken: Did not manage to get serial from device, using XX01 instead\n");
			device->serial[0] = 'X';
			device->serial[0] = 'X';
			device->serial[0] = '0';
			device->serial[0] = '1';
				     ...
				     struct device_attribute *attr, char *buf)
{
	struct razer_kraken_device *device = dev_get_drvdata(dev);
	struct razer_kraken_request_report report =
		get_kraken_request_report(0x04, 0x20, 0x02, 0x0030);
	...
	if (device->firmware_version[0] != 1) {
		...
		if (device->data[0] == 0x05) {
			...
			printk(KERN_CRIT
			       "razerkraken: Did not manage to get firmware version from device, using v9.99 instead\n");
			device->firmware_version[0] = 1;
 ...
 */
static ssize_t
razer_attr_read_matrix_current_effect(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(matrix_current_effect, 0440,
		   razer_attr_read_matrix_current_effect, NULL);
...
static DEVICE_ATTR(matrix_effect_static, 0660, razer_attr_read_mode_static,
		   razer_attr_write_mode_static);
static DEVICE_ATTR(matrix_effect_custom, 0660, razer_attr_read_mode_custom,
		   razer_attr_write_mode_custom);
static DEVICE_ATTR(matrix_effect_breath, 0660, razer_attr_read_mode_breath,
		   razer_attr_write_mode_breath);

void razer_kraken_init(struct razer_kraken_device *dev,
 ...
 */
static int razer_kraken_probe(struct hid_device *hdev,
			      const struct hid_device_id *id)
{
	...
	if (dev->usb_interface_protocol == USB_INTERFACE_PROTOCOL_NONE) {
		CREATE_DEVICE_FILE(&hdev->dev,
				   &dev_attr_version); // Get driver version
		...
		CREATE_DEVICE_FILE(
			&hdev->dev,
			&dev_attr_device_serial); // Get string of device serial
		CREATE_DEVICE_FILE(
			&hdev->dev,
			&dev_attr_firmware_version); // Get string of device fw version
		CREATE_DEVICE_FILE(&hdev->dev,
		...
		case USB_DEVICE_ID_RAZER_KRAKEN_CLASSIC_ALT:
			...
			CREATE_DEVICE_FILE(
				&hdev->dev,
				&dev_attr_matrix_current_effect); // Get current effect
			break;
		case USB_DEVICE_ID_RAZER_KRAKEN:
		case USB_DEVICE_ID_RAZER_KRAKEN_V2:
			CREATE_DEVICE_FILE(
			...
			CREATE_DEVICE_FILE(
				&hdev->dev,
				&dev_attr_matrix_effect_breath); // Brething effect
			CREATE_DEVICE_FILE(
				&hdev->dev,
				&dev_attr_matrix_current_effect); // Get current effect
			break;
			   ...
			   u8 *data, int size)
{
	...
	if (size == 33) {
		...
		printk(KERN_WARNING
		       "razerkraken: Got raw message, length: %d\n",
		       size);
	}
...
static const struct hid_device_id razer_devices[] = {
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_KRAKEN_CLASSIC) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
