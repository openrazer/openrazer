...
int razer_get_usb_response(struct usb_device *usb_dev, uint report_index,
			   ...
			   uint response_index,
			   struct razer_report *response_report, ulong wait_min,
			   ulong wait_max)
{
	...
	retval = razer_send_control_msg(usb_dev, request_report, report_index,
					wait_min, wait_max);
	...
	if (len != 90) {
		printk(KERN_WARNING
		       "razer driver: Invalid USB response. USB Report length: %d\n",
		       len);
		result = 1;
	}
			    ...
			    char *message)
{
	printk(KERN_WARNING
	       ...
	       driver_name, message, report->status, report->transaction_id.id,
	       report->data_size, report->command_class, report->command_id.id,
