 ...
 * Version Information
 */
#define DRIVER_DESC "Razer Core Device Driver"
		     ...
		     struct razer_report *response_report)
{
	return razer_get_usb_response(usb_dev, 0x00, request_report, 0x00,
				      response_report, RAZER_CORE_WAIT_MIN_US,
				      RAZER_CORE_WAIT_MAX_US);
}
					      ...
					      const char *buf, size_t count)
{
	...
	if (count == 4) {
		...
		printk(KERN_WARNING
		       "razercore: Reactive only accepts Speed, RGB (4byte)");
	}
	return count;
}
 ...
 *
 * Sets the core to custom mode whenever the file is written to
 */
 ...
 *
 * Set the core to static mode when 3 RGB bytes are written
 */
					    ...
					    const char *buf, size_t count)
{
	...
	if (count == 3) {
		...
		printk(KERN_WARNING
		       "razercore: Static mode only accepts RGB (3byte)");
	}
 ...
 *
 * Writes the colour to the LEDs of the core
 *
 * Start is 0x00
 * Stop is 0x0E
 */
					    ...
					    const char *buf, size_t count)
{
	...
	//printk(KERN_ALERT "razercore: Total count: %d\n", (unsigned char)count);

	while (offset < count) {
		if (offset + 3 > count) {
			printk(KERN_ALERT
			       "razercore: Wrong Amount of data provided: Should be ROW_ID, START_COL, STOP_COL, N_RGB\n");
			break;
		}
		...
		// printk(KERN_ALERT "razercore: Row ID: %d, Start: %d, Stop: %d, row length: %d\n", row_id, start_col, stop_col, row_length);

		if (row_id != 0) {
			printk(KERN_ALERT "razercore: Row ID must be 0\n");
			break;
		}

		if (start_col > stop_col) {
			printk(KERN_ALERT
			       "razercore: Start column is greater than end column\n");
			break;
		}

		if (offset + row_length > count) {
			printk(KERN_ALERT
			       "razercore: Not enough RGB to fill row\n");
			break;
		}
					    ...
					    const char *buf, size_t count)
{
	...
	if (count == 2) {
		...
		printk(KERN_WARNING
		       "razercore: Device mode only takes 2 bytes.");
	}
...
static DEVICE_ATTR(matrix_brightness, 0664, razer_attr_read_set_brightness,
 ...
 * TODO remove goto's
 */
static int razer_core_probe(struct hid_device *hdev,
 ...
 */
static struct hid_driver razer_core_driver = {
	.name = "razercore",
	.id_table = razer_devices,
	.probe = razer_core_probe,
	.remove = razer_core_disconnect,
};

module_hid_driver(razer_core_driver);
