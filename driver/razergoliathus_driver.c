...
#include "razergoliathus_driver.h"
 ...
 * Version Information
 */
#define DRIVER_DESC "Razer Goliathus Device Driver"
...
{
    return razer_get_usb_response(usb_dev, 0x00, request_report, 0x00, response_report, RAZER_GOLIATHUS_WAIT_MIN_US, RAZER_GOLIATHUS_WAIT_MAX_US);
}
    ...
    if(retval == 0) {
        ...
        if(response_report.remaining_packets != request_report->remaining_packets ||
           ...
           response_report.command_id.id != request_report->command_id.id) {
            print_erroneous_report(&response_report, "razergoliathus", "Response doesn't match request");
        } else if (response_report.status == RAZER_CMD_FAILURE) {
            print_erroneous_report(&response_report, "razergoliathus", "Command failed");
        } else if (response_report.status == RAZER_CMD_NOT_SUPPORTED) {
            print_erroneous_report(&response_report, "razergoliathus", "Command not supported");
        } else if (response_report.status == RAZER_CMD_TIMEOUT) {
            print_erroneous_report(&response_report, "razergoliathus", "Command timed out");
        }
    } else {
        print_erroneous_report(&response_report, "razergoliathus", "Invalid Report Length");
    }
...
static ssize_t razer_attr_read_set_brightness(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    struct razer_report report = razer_chroma_extended_matrix_get_brightness(VARSTORE, 0x1);
...
static ssize_t razer_attr_write_set_brightness(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    report = razer_chroma_extended_matrix_brightness(VARSTORE, 0x1, brightness);
...
{
    return sprintf(buf, "Razer Goliathus\n");
}
...
static ssize_t razer_attr_write_mode_none(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    struct usb_interface *intf = to_usb_interface(dev->parent);
    struct usb_device *usb_dev = interface_to_usbdev(intf);
    struct razer_report report = razer_chroma_extended_matrix_effect_none(VARSTORE, 0x1);
    razer_send_payload(usb_dev, &report);
...
static ssize_t razer_attr_write_mode_spectrum(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    struct usb_interface *intf = to_usb_interface(dev->parent);
    struct usb_device *usb_dev = interface_to_usbdev(intf);
    struct razer_report report = razer_chroma_extendex_matrix_effect_spectrum(VARSTORE, 0x1);
    razer_send_payload(usb_dev, &report);
...
static ssize_t razer_attr_write_mode_reactive(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if(count == 4) {
        ...
        report = razer_chroma_extended_matrix_effect_reactive(VARSTORE, 0x1, speed, (struct razer_rgb*)&buf[1]);

        razer_send_payload(usb_dev, &report);

    } else {
        printk(KERN_WARNING "razergoliathus: Reactive only accepts Speed, RGB (4byte)");
    }
    return count;
}
...
static ssize_t razer_attr_write_mode_breath(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    case 3: // Single colour mode
        report = razer_chroma_extended_matrix_effect_breathing_single(VARSTORE, 0x1, (struct razer_rgb*)&buf[0]);
        razer_send_payload(usb_dev, &report);
        break;

    case 6: // Dual colour mode
        report = razer_chroma_extended_matrix_effect_breathing_dual(VARSTORE, 0x1, (struct razer_rgb*)&buf[0], (struct razer_rgb*)&buf[3]);
        razer_send_payload(usb_dev, &report);
        break;

    default: // "Random" colour mode
        report = razer_chroma_extended_matrix_effect_breathing_random(VARSTORE, 0x1);
        razer_send_payload(usb_dev, &report);
        break;
...
static ssize_t razer_attr_write_mode_custom(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    struct usb_interface *intf = to_usb_interface(dev->parent);
    struct usb_device *usb_dev = interface_to_usbdev(intf);
    struct razer_report report = razer_chroma_extended_matrix_effect_custom_frame(NOSTORE);
    razer_send_payload(usb_dev, &report);
    return count;
}
...
static ssize_t razer_attr_write_mode_static(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if(count == 3) {
        report = razer_chroma_extended_matrix_effect_static(VARSTORE, 0x1, (struct razer_rgb*)&buf[0]);
        razer_send_payload(usb_dev, &report);
    } else {
        printk(KERN_WARNING "razergoliathus: Static mode only accepts RGB (3byte)");
    }
...
static ssize_t razer_attr_write_set_key_row(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    while(offset < count) {
        if(offset + 3 > count) {
            printk(KERN_ALERT "razergoliathus: Wrong Amount of data provided: Should be ROW_ID, START_COL, STOP_COL, N_RGB\n");
            break;
        }
...
static ssize_t razer_attr_write_device_mode(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if(count == 2) {
        ...
        printk(KERN_WARNING "razergoliathus: Device mode only takes 2 bytes.");
    }
 ...
 * TODO remove goto's
 */
static int razer_goliathus_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
    int retval = 0;
    struct usb_interface *intf = to_usb_interface(hdev->dev.parent);
    struct usb_device *usb_dev = interface_to_usbdev(intf);
    struct razer_goliathus_device *dev = NULL;

    dev = kzalloc(sizeof(struct razer_goliathus_device), GFP_KERNEL);
    if(dev == NULL) {
 ...
 * Unbind function
 */
static void razer_goliathus_disconnect(struct hid_device *hdev)
{
 ...
 */
static struct hid_driver razer_goliathus_driver = {
    .name =         "razergoliathus",
    .id_table =     razer_devices,
    .probe =        razer_goliathus_probe,
    .remove =       razer_goliathus_disconnect,
};

module_hid_driver(razer_goliathus_driver);
