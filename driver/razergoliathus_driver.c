		     ...
		     struct razer_report *response_report)
{
	return razer_get_usb_response(usb_dev, 0x00, request_report, 0x00,
				      response_report,
				      RAZER_GOLIATHUS_WAIT_MIN_US,
				      RAZER_GOLIATHUS_WAIT_MAX_US);
}
	...
	if (retval == 0) {
		...
		if (response_report.remaining_packets !=
			...
			print_erroneous_report(
				&response_report, "razergoliathus",
				"Response doesn't match request");
		} else if (response_report.status == RAZER_CMD_FAILURE) {
			print_erroneous_report(&response_report,
					       "razergoliathus",
					       "Command failed");
		...
		print_erroneous_report(&response_report, "razergoliathus",
 ...
 * TODO remove goto's
 */
static int razer_goliathus_probe(struct hid_device *hdev,
...
static const struct hid_device_id razer_devices[] = {
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_GOLIATHUS_CHROMA) },
	{ HID_USB_DEVICE(USB_VENDOR_ID_RAZER,
			 USB_DEVICE_ID_RAZER_GOLIATHUS_CHROMA_EXTENDED) },
	{}
};
