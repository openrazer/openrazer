	...
	if (retval == 0) {
		...
		if (response_report.remaining_packets !=
			...
			print_erroneous_report(
				&response_report, "razermug",
				"Response doesn't match request");
		} else if (response_report.status == RAZER_CMD_FAILURE) {
			print_erroneous_report(&response_report, "razermug",
					       "Command failed");
					      ...
					      const char *buf, size_t count)
{
	struct razer_mug_device *device = dev_get_drvdata(dev);
	struct razer_report report =
		razer_chroma_standard_matrix_effect_spectrum(VARSTORE,
							     BACKLIGHT_LED);
	report.transaction_id.id = 0x3F;
...
{
	struct razer_mug_device *device = dev_get_drvdata(dev);
	struct razer_report report = razer_chroma_standard_matrix_effect_none(
 ...
 */
static ssize_t razer_attr_write_mode_blinking(struct device *dev,
					      struct device_attribute *attr,
					      const char *buf, size_t count)
{
	struct razer_mug_device *device = dev_get_drvdata(dev);
	struct razer_report report_rgb;
	struct razer_report report_effect =
		razer_chroma_standard_set_led_effect(VARSTORE, BACKLIGHT_LED,
						     0x01);
	report_effect.transaction_id.id = 0x3F;

	if (count == 3) {
		report_rgb = razer_chroma_standard_set_led_rgb(
			VARSTORE, BACKLIGHT_LED, (struct razer_rgb *)&buf[0]);
		...
		printk(KERN_WARNING
		       "razermug: Blinking mode only accepts RGB (3byte)\n");
	}
					    ...
					    const char *buf, size_t count)
{
	...
	if (count == 3) {
		...
		printk(KERN_WARNING
		       "razermug: Static mode only accepts RGB (3byte)\n");
	}
					    ...
					    const char *buf, size_t count)
{
	...
	case 3: // Single colour mode
		report = razer_chroma_standard_matrix_effect_breathing_single(
			VARSTORE, BACKLIGHT_LED, (struct razer_rgb *)&buf[0]);
		break;

	case 6: // Dual colour mode
		report = razer_chroma_standard_matrix_effect_breathing_dual(
			VARSTORE, BACKLIGHT_LED, (struct razer_rgb *)&buf[0],
			(struct razer_rgb *)&buf[3]);
		break;

	default: // "Random" colour mode
		report = razer_chroma_standard_matrix_effect_breathing_random(
			VARSTORE, BACKLIGHT_LED);
		break;
	}
	// Set device id
					    ...
					    const char *buf, size_t count)
{
	...
	while (offset < count) {
		if (offset + 3 > count) {
			printk(KERN_ALERT
			       "razermug: Wrong Amount of data provided: Should be ROW_ID, START_COL, STOP_COL, N_RGB\n");
			break;
		}
		...
		if (start_col > stop_col) {
			printk(KERN_ALERT
			       "razermug: Start column is greater than end column\n");
			break;
		}

		if (offset + row_length > count) {
			printk(KERN_ALERT
			       "razermug: Not enough RGB to fill row\n");
			break;
		}

		report = razer_chroma_misc_one_row_set_custom_frame(
			start_col, stop_col, (unsigned char *)&buf[offset]);
...
static ssize_t razer_attr_read_get_cup_state(struct device *dev,
					     struct device_attribute *attr,
					     char *buf)
{
	struct razer_mug_device *device = dev_get_drvdata(dev);
					       ...
					       const char *buf, size_t count)
{
	...
	if (count > 0) {
		...
		printk(KERN_WARNING
		       "razermug: Brightness takes an ascii number\n");
	}
...
static DEVICE_ATTR(matrix_effect_blinking, 0220, NULL,
		   razer_attr_write_mode_blinking);
...
void razer_mug_init(struct razer_mug_device *dev, struct usb_interface *intf,
 ...
 */
static int razer_input_mapping(struct hid_device *hdev, struct hid_input *hi,
 ...
 */
static int razer_mug_probe(struct hid_device *hdev,
			   const struct hid_device_id *id)
{
	...
	if (dev->usb_interface_protocol == USB_INTERFACE_PROTOCOL_MOUSE) {
		...
		CREATE_DEVICE_FILE(
			&hdev->dev,
			&dev_attr_device_mode); // Get string of device mode
		CREATE_DEVICE_FILE(
			&hdev->dev,
		...
		CREATE_DEVICE_FILE(&hdev->dev,
				   &dev_attr_is_mug_present); // Is cup present

		CREATE_DEVICE_FILE(
			&hdev->dev,
			&dev_attr_matrix_custom_frame); // Custom effect frame
		CREATE_DEVICE_FILE(&hdev->dev,
		...
		CREATE_DEVICE_FILE(
			&hdev->dev,
			&dev_attr_matrix_effect_blinking); // Blinking effect
		CREATE_DEVICE_FILE(&hdev->dev,
				   &dev_attr_matrix_brightness); // Brightness
 ...
 */
static struct hid_driver razer_mug_driver = { .name = "razermug",
