...
while IFS= read -r device_raw; do
    ...
    # HyperFlux V2's raw receiver/mat interface is kernel-driver-only; the
    # daemon exposes the paired keyboard and mouse interfaces instead.
    if [ "$driver" = "razeraccessory" ] && [ "$device_pid" = "00CF" ]; then
        continue
    fi
    ...
    device_dir_suffix="0001"
    hyperflux_interface=
    if [ "$device_pid" = "00CF" ]; then
        if [ "$driver" = "razerkbd" ]; then
            device_dir_suffix="0002"
            hyperflux_interface="01"
        elif [ "$driver" = "razermouse" ]; then
            device_dir_suffix="0004"
            hyperflux_interface="04"
        fi
    fi
...
[device]
dir_name = 0003:1532:$device_pid.$device_dir_suffix
name = $device_name
EOF
    ...
    while IFS= read -r attr; do
        ...
        if [[ "$attr" = "dpi" && " ${byte_dpi_devices[@]} " =~ " ${device_pid} " ]]; then
            ...
            if [ -n "$hyperflux_interface" ]; then
                default="XX0000${device_pid}${device_dir_suffix#00}"
            fi
        fi
    ...
    if [ -n "$hyperflux_interface" ]; then
        echo "        r,bInterfaceNumber,$hyperflux_interface" >> "$filename"
    fi
done <<< "$devices"
