...
class EffectSync(object):
    ...
    def __init__(self, parent, device_number):
        self._logger = logging.getLogger(
            'razer.device{0}.effect_sync'.format(device_number))
        self._parent = parent
        ...
        try:
            ...
            if effect_func is not None:
                ...
                if not effect_name == 'setNone':
                    effect_func = getattr(
                        self._parent, 'setScrollActive', None)
                    if effect_func is not None:
                        effect_func(True)
                    ...
                    if effect_func is not None:
                        effect_func(True)
                    effect_func = getattr(
                        self._parent, 'setBacklightActive', None)
                    if effect_func is not None:
                        effect_func(True)
                ...
                if effect_name == 'setPulsate':
                    ...
                    effect_func = getattr(
                        self._parent, 'setBreathSingle', None)
                    if effect_func is not None:
                        effect_func(*pargs)
                    effect_func = getattr(
                        self._parent, 'setScrollPulsate', None)
                    if effect_func is not None:
                        effect_func(*pargs)
                    ...
                    if effect_func is not None:
                        effect_func(*pargs)
                    effect_func = getattr(
                        self._parent, 'setBacklightPulsate', None)
                    if effect_func is not None:
                        effect_func(*pargs)
                    ...
                    if effect_name == 'setBreathRandom':
                        ...
                        # limit args to first 3, as setBreathDual gives 6 args
                        pargs = args[0:3]
                    ...
                    effect_func = getattr(
                        self._parent, 'setScrollSpectrum', None)
                    if effect_func is not None:
                        effect_func()
                    effect_func = getattr(
                        self._parent, 'setLogoSpectrum', None)
                    if effect_func is not None:
                        effect_func()
                    effect_func = getattr(
                        self._parent, 'setBacklightSpectrum', None)
                    if effect_func is not None:
                        effect_func()

                elif effect_name == 'setStatic':
                    effect_func = getattr(
                        self._parent, 'setScrollStatic', None)
                    if effect_func is not None:
                        effect_func(*args)
                    ...
                    if effect_func is not None:
                        effect_func(*args)
                    effect_func = getattr(
                        self._parent, 'setBacklightStatic', None)
                    if effect_func is not None:
                        effect_func(*args)

        except Exception as err:
            self._logger.exception(
                "Caught exception trying to sync effects.", exc_info=err)
