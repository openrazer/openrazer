...
HARDWARE_MODULES = ['openrazer_daemon.hardware.' + os.path.splitext(hw_file)[0] for hw_file in os.listdir(
    os.path.dirname(__file__)) if hw_file not in ('device_base.py', '__init__.py') and hw_file.endswith('.py')]
...
def get_device_classes():
    ...
    for hw_module in HARDWARE_MODULES:
        imported_module = __import__(
            hw_module, globals=globals(), locals=locals(), fromlist=['*'], level=0)
