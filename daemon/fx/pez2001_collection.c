...
int effect1_update(struct razer_fx_render_node *render)
{
	...
	for(x=0;x<22;x++)
		for(y=0;y<6;y++)
		{
			...
			rgb_mix_into(&render->output_frame->rows[y].column[x],&color,&render->input_frame->rows[y].column[x],0.1f);
...
int effect2_update(struct razer_fx_render_node *render)
{
	...
	for(x=0;x<22;x++)
		for(y=0;y<6;y++)
		{
			...
			rgb_mix_into(&render->output_frame->rows[y].column[x],&color,&render->input_frame->rows[y].column[x],0.5f);
			render->output_frame->update_mask |= 1<<y;
		}
	count+=dir;
	if(count<1 || count>99)
...
void effect3_update(struct razer_fx_render_node *render)
{
	...
	for(x=0;x<22;x++)
	{
		for(y=0;y<6;y++)
		{
			...
			rgb_mix_into(&render->output_frame->rows[y].column[x],&color,&render->input_frame->rows[y].column[x],0.6f);
			render->output_frame->update_mask |= 1<<y;
		}
	}
	count+=dir;
...
void effect4_update(struct razer_fx_render_node *render)
{
	...
	for(x=0;x<22;x++)
		for(y=0;y<6;y++)
		{
			...
			rgb_mix_into(&render->output_frame->rows[y].column[x],&color,&render->input_frame->rows[y].column[x],0.4f);
			render->output_frame->update_mask |= 1<<y;
		}
...
void effect7_update(struct razer_fx_render_node *render)
{
	...
	razer_mix_frames(render->output_frame,render->input_frame,0.5f);
	count=count+dir;
	if(count>27 || count<-7)
		dir=-dir;
	daemon_set_parameter_int(daemon_get_parameter_by_index(render->effect->parameters,0),count);	
