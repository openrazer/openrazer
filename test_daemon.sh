...
RUID=`sudo dbus-send --type=method_call --print-reply=literal --dest=org.voyagerproject.razer.daemon / org.voyagerproject.razer.daemon.render_node.create int32:1 string:"Ext Node 2" | jq '.uid'`
#[ $? -eq 0 ] || echo "no daemon found";exit $?;
echo "setting render node uid : $RUID"

#set render node to compute/update by uid
